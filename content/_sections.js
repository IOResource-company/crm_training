function quickStartHTML(){
  const actions = [
    ...
    ['Email it in when that is easier', 'The quickest way to get a lead, an update or a task into the CRM is to <strong>forward the email to sales@ioresource.com and type one word: <code>Sales</code></strong>. The automation reads the whole mail and files it against the right deal, or raises a review task if it is not certain. Add <code>Sales lead</code>, <code>Sales task</code> or <code>Sales note</code> to say exactly what you want. No browser, no fields &mdash; it works from a phone.'],
